...
pub const PROG_MAX_INSNS: usize = 1_000_000;
...
/// Extra 8 bytes for canary protection.
pub const STACK_SIZE: usize = 8192 + 8;
...
pub type HelperWithCtx = fn(usize, u64, u64, u64, u64, u64) -> HelperResult;
pub type HelperResult = Result<u64, String>;
...
pub fn to_insn_vec(prog: &[u8]) -> Vec<Insn> {
	if prog.len() % INSN_SIZE != 0 {
		panic!("Error: eBPF program length must be a multiple of {:?} octets", INSN_SIZE);
	}
