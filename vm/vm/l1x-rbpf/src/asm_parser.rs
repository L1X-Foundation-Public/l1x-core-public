...
use combine::{
	...
	primitives::{Error, Info},
	sep_by, try, ParseError, ParseResult, Parser, State, Stream,
};
	...
	I: Stream<Item = char>,
{
	...
	let memory = between(char('['), char(']'), (parser(register), optional(parser(integer))))
	...
	I: Stream<Item = char>,
{
	...
	(parser(ident).skip(spaces()), operands, spaces())
		.map(|t| Instruction { name: t.0, operands: t.1 })
		.parse_stream(input)
}
...
mod tests {
	...
	fn test_integer() {
		...
		assert_eq!(parser(integer).parse("0x123456789abcdef0"), Ok((0x123456789abcdef0, "")));
	...
	fn test_operand() {
		...
		assert_eq!(parser(operand).parse("r15"), Ok((Operand::Register(15), "")));
		assert_eq!(parser(operand).parse("0"), Ok((Operand::Integer(0), "")));
		assert_eq!(parser(operand).parse("42"), Ok((Operand::Integer(42), "")));
		assert_eq!(parser(operand).parse("[r1]"), Ok((Operand::Memory(1, 0), "")));
		assert_eq!(parser(operand).parse("[r3+5]"), Ok((Operand::Memory(3, 5), "")));
		assert_eq!(parser(operand).parse("[r3+0x1f]"), Ok((Operand::Memory(3, 31), "")));
		assert_eq!(parser(operand).parse("[r3-0x1f]"), Ok((Operand::Memory(3, -31), "")));
	}
	...
	fn test_instruction() {
		assert_eq!(
			...
			Ok((Instruction { name: "exit".to_string(), operands: vec![] }, ""))
		);

		assert_eq!(
			parser(instruction).parse("call 2"),
			Ok((Instruction { name: "call".to_string(), operands: vec![Operand::Integer(2)] }, ""))
		);
		...
		assert_eq!(
			...
			Ok((
				Instruction {
					name: "jne".to_string(),
					operands: vec![Operand::Register(3), Operand::Integer(8), Operand::Integer(37)],
				},
				""
			))
		);
	...
	fn test_exit() {
		...
		assert_eq!(
			parse("exit"),
			Ok(vec![Instruction { name: "exit".to_string(), operands: vec![] }])
		);
	}
	...
	fn test_ja() {
		...
		assert_eq!(
			parse("ja +1"),
			Ok(vec![Instruction { name: "ja".to_string(), operands: vec![Operand::Integer(1)] }])
		);
	}
...
";
		...
		assert_eq!(
			...
			Ok(vec![
				...
				Instruction {
					name: "jne".to_string(),
					operands: vec![Operand::Register(2), Operand::Integer(6), Operand::Integer(35)],
				},
				Instruction {
					name: "ldxb".to_string(),
				...
				Instruction {
					name: "jeq".to_string(),
					operands: vec![Operand::Register(5), Operand::Integer(1), Operand::Integer(4)],
				},
				Instruction {
					name: "jeq".to_string(),
					operands: vec![Operand::Register(5), Operand::Integer(0), Operand::Integer(12)],
				},
				Instruction {
					name: "mov".to_string(),
				...
				Instruction {
					name: "jeq".to_string(),
					operands: vec![Operand::Register(5), Operand::Integer(5), Operand::Integer(9)],
				},
				Instruction { name: "ja".to_string(), operands: vec![Operand::Integer(2)] },
				Instruction {
					name: "add".to_string(),
				...
				Instruction { name: "ja".to_string(), operands: vec![Operand::Integer(1)] },
				Instruction {
					name: "mov".to_string(),
					operands: vec![Operand::Register(0), Operand::Integer(1)],
				},
				Instruction { name: "exit".to_string(), operands: vec![] }
			])
		);
	}
	...
	fn test_error_unexpected_character() {
		...
		assert_eq!(
			parse("exit\n^"),
			Err("Parse error at line 2 column 1: unexpected '^', expected end of input".to_string())
		);
	}
