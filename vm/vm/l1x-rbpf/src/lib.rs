...
pub struct EbpfVmMbuff<'a> {
	...
	helpers_with_ctx: HashMap<u32, ebpf::HelperWithCtx>,
	ctx: usize,
}
...
impl<'a> EbpfVmMbuff<'a> {
	...
	pub fn register_helper_with_ctx(
		&mut self,
		key: u32,
		function: HelperWithCtx,
	) -> Result<(), Error> {
		self.helpers_with_ctx.insert(key, function);
		Ok(())
	}

	pub fn set_ctx(&mut self, ctx: usize) -> Result<(), Error> {
		self.ctx = ctx;
		Ok(())
	}
	...
	pub fn execute_program(
		...
		mbuff: &[u8],
		start_offset: u64,
		fee: execution_fee::Fee,
		mut runtime_gas: Rc<RefCell<runtime_config::RuntimeGas>>,
	) -> Result<u64, anyhow::Error> {
		const U32MAX: u64 = u32::MAX as u64;

		let prog = match self.prog {
			Some(prog) => prog,
			None => Err(EbpfVmError::VMExecNoProgramSet)?,
		};
		const MAX_CALL_DEPTH: usize = 50;
		let mut stack = vec![0u8; MAX_CALL_DEPTH * ebpf::STACK_SIZE];
		#[derive(Default, Clone)]
		pub struct CallFrame {
			pub caller_saved_registers: [u64; 4],
		...
		let mut call_frames: Vec<CallFrame> = vec![CallFrame::default(); MAX_CALL_DEPTH];
		...
		let check_mem_load =
			|addr: u64, len: usize, insn_ptr: usize, stack: &[u8]| -> Result<(), EbpfVmError> {
				EbpfVmMbuff::check_mem(addr, len, "load", insn_ptr, mbuff, mem, stack)
					.map_err(|e_val| EbpfVmError::VMExecMemoryOutOfBound(e_val))?;
				Ok(())
			};
		let check_mem_store =
			|addr: u64, len: usize, insn_ptr: usize, stack: &[u8]| -> Result<(), EbpfVmError> {
		...
		let mut insn_ptr: usize = start_offset as usize / ebpf::INSN_SIZE;
		while insn_ptr * ebpf::INSN_SIZE < prog.len() {
			if runtime_gas.borrow().left() == 0 {
				Err(EbpfVmError::VMOutOfGas(runtime_gas.borrow().gas_limit))?;
			}
			...
			let mut do_jump = || {
				insn_ptr = (insn_ptr as i64 + insn.off as i64) as usize;
			};
			...
			match insn.opc {
				...
				ebpf::LD_ABS_B => {
					reg[0] = unsafe {
						...
						check_mem_load(x as u64, 8, insn_ptr, &stack)?;
						*x as u64
					}
				},
				ebpf::LD_ABS_H => {
				...
				ebpf::LD_IND_B => {
					reg[0] = unsafe {
						let x = (mem.as_ptr() as u64 + reg[_src] + (insn.imm as u32) as u64)
							as *const u8;
						check_mem_load(x as u64, 8, insn_ptr, &stack)?;
						*x as u64
					}
				...
				ebpf::LD_B_REG => {
					reg[_dst] = unsafe {
						...
						check_mem_load(x as u64, 1, insn_ptr, &stack)?;
						*x as u64
					}
				},
				ebpf::LD_H_REG => {
					reg[_dst] = unsafe {
						...
						check_mem_load(x as u64, 2, insn_ptr, &stack)?;
						*x as u64
					}
				},
				ebpf::LD_W_REG => {
					reg[_dst] = unsafe {
						...
						check_mem_load(x as u64, 4, insn_ptr, &stack)?;
						*x as u64
					}
				},
				ebpf::LD_DW_REG => {
				...
				ebpf::ADD32_IMM => reg[_dst] = (reg[_dst] as u32).wrapping_add(insn.imm as u32) as u64, //((reg[_dst] & U32MAX) + insn.imm  as u64)     & U32MAX,
				ebpf::ADD32_REG => {
					reg[_dst] = (reg[_dst] as u32).wrapping_add(reg[_src] as u32) as u64
				}, //((reg[_dst] & U32MAX) + (reg[_src] & U32MAX)) & U32MAX,
				ebpf::SUB32_IMM => reg[_dst] = (reg[_dst] as u32).wrapping_sub(insn.imm as u32) as u64,
				ebpf::SUB32_REG => {
					reg[_dst] = (reg[_dst] as u32).wrapping_sub(reg[_src] as u32) as u64
				},
				ebpf::MUL32_IMM => reg[_dst] = (reg[_dst] as u32).wrapping_mul(insn.imm as u32) as u64,
				ebpf::MUL32_REG => {
					reg[_dst] = (reg[_dst] as u32).wrapping_mul(reg[_src] as u32) as u64
				},
				...
				ebpf::LSH64_IMM => reg[_dst] = reg[_dst].wrapping_shl(insn.imm as u32),
				ebpf::LSH64_REG => {
					let shift = reg[_src] % 64;
					reg[_dst] <<= shift;
				},
				ebpf::RSH64_IMM => reg[_dst] = reg[_dst].wrapping_shr(insn.imm as u32),
				ebpf::RSH64_REG => {
					let shift = reg[_src] % 64;
					reg[_dst] >>= shift
				},
				...
				ebpf::CALL => {
					...
					if insn.src == 0 {
						if let Some(function) = self.helpers_with_ctx.get(&(insn.imm as u32)) {
							reg[0] = function(self.ctx, reg[1], reg[2], reg[3], reg[4], reg[5])
								.map(|r_val| r_val)
								.map_err(|e_val| EbpfVmError::VMExecCtxCallError(e_val))?;

							runtime_gas.borrow_mut().burn(fee.api_call);
						} else {
							Err(EbpfVmError::VMExecCtxInvalidHelperFunctionId(insn.imm as u64))?;
						}
					} else {
						// println!("CALL: {:#x}", insn.imm);
						if call_depth >= call_frames.len() {
							let insn_offset = insn_ptr * INSN_SIZE;
							Err(EbpfVmError::VMExecCallDepthMax(insn_ptr, insn_offset))?;
						}

						let canary: u64 = rand::random();
						let frame = &mut call_frames[call_depth];
						frame.caller_saved_registers.copy_from_slice(&reg[6..10]);
						frame.frame_pointer = reg[10];
						frame.target_pc = insn_ptr;
						frame.canary = canary;
						...
						// // Set up canary protection
						// let stack_ptr = stack.as_ptr() as usize;
						// let stack_idx = (reg[10] as usize) - stack_ptr;
						// let canary_slice = stack
						//     .as_mut_slice()
						//     .get_mut(stack_idx - 8..stack_idx)
						//     .unwrap();
						// LittleEndian::write_u64(canary_slice, canary);
						// // It is safe to move r10 by 8 as STACK_SIZE has extra 8 bytes allocated for canary protection.
						// reg[10] -= 8;

						insn_ptr = if insn.imm.is_negative() {
							insn_ptr - insn.imm.wrapping_abs() as u32 as usize
						} else {
				...
				ebpf::EXIT => {
					...
					call_depth -= 1;
					let frame = &call_frames[call_depth];

					// // Check that canary value is still there
					// let stack_idx = (reg[10] as usize) - (stack.as_ptr() as usize);
					// let canary_slice = stack.as_slice().get(stack_idx..stack_idx + 8).unwrap();
					// let canary = LittleEndian::read_u64(canary_slice);
					// if canary != frame.canary {
					//     return Err(Error::new(ErrorKind::Other, "buffer overflow"));
					// }

					insn_ptr = frame.target_pc;
					reg[10] = frame.frame_pointer;
					reg[6..10].copy_from_slice(&frame.caller_saved_registers);
				},

				_ => unreachable!(),
			}

			runtime_gas.borrow_mut().burn(fee.basic_instruction);

			if insn_ptr > prog.len() / ebpf::INSN_SIZE {
				println!("Last instuction: {:0x?} {:0x?}", insn, insn.to_array());
				println!("{insn_ptr} {}", prog.len());
			}
		}
		unreachable!()
	}
	...
	fn check_mem(
		...
		if mbuff.as_ptr() as u64 <= addr &&
			addr + len as u64 <= mbuff.as_ptr() as u64 + mbuff.len() as u64
		{
			return Ok(())
		}
		if mem.as_ptr() as u64 <= addr &&
			addr + len as u64 <= mem.as_ptr() as u64 + mem.len() as u64
		{
			return Ok(())
		}
		if stack.as_ptr() as u64 <= addr &&
			addr + len as u64 <= stack.as_ptr() as u64 + stack.len() as u64
		{
			return Ok(())
		}
	...
	pub unsafe fn execute_program_jit(
		...
		match self.jit {
			...
			None => Err(Error::new(ErrorKind::Other, "Error: program has not been JIT-compiled")),
		}
	}
}
...
/// packet data in the buffer. On calling the `execute_program()` or `execute_program_jit()`
/// functions, the struct automatically updates the addresses in this static buffer, at the
/// appointed offsets, for the start and the end of the packet data the program is called upon.
///
/// # Examples
///
...
impl<'a> EbpfVmFixedMbuff<'a> {
	...
	pub fn new(
		...
		let mbuff = MetaBuff { data_offset, data_end_offset, buffer };
	...
	/// assert_eq!(res, 0xdd);
	/// ```
	pub fn execute_program(&mut self, mem: &'a mut [u8]) -> Result<u64, anyhow::Error> {
		...
		if self.mbuff.data_offset + 8 > l || self.mbuff.data_end_offset + 8 > l {
			Err(EbpfVmError::VMExecMBuffTooSmall(
				l,
		...
		self.parent.execute_program(
			mem,
			&self.mbuff.buffer,
			0,
			execution_fee::Fee::create_basic_config(),
			RefCell::new(runtime_config::RuntimeGas::new_unlimited()).into(),
		)
	}
...
impl<'a> EbpfVmRaw<'a> {
	...
	/// assert_eq!(res, 0x22cc);
	/// ```
	pub fn execute_program(&self, mem: &'a mut [u8]) -> Result<u64, anyhow::Error> {
		self.parent.execute_program(
			mem,
			&[],
			0,
...
impl<'a> EbpfVmNoData<'a> {
	...
	/// assert_eq!(res, 0x1122);
	/// ```
	pub fn execute_program(&self) -> Result<u64, anyhow::Error> {
