...
fn make_instruction_map() -> HashMap<String, (InstructionType, u8)> {
...

	{
		...
		for &(suffix, size) in &mem_sizes {
			entry(&format!("ldabs{suffix}"), LoadAbs, ebpf::BPF_ABS | ebpf::BPF_LD | size);
			entry(&format!("ldind{suffix}"), LoadInd, ebpf::BPF_IND | ebpf::BPF_LD | size);
			entry(&format!("ldx{suffix}"), LoadReg, ebpf::BPF_MEM | ebpf::BPF_LDX | size);
			entry(&format!("st{suffix}"), StoreImm, ebpf::BPF_MEM | ebpf::BPF_ST | size);
			entry(&format!("stx{suffix}"), StoreReg, ebpf::BPF_MEM | ebpf::BPF_STX | size);
		}
...
fn insn(opc: u8, dst: i64, src: i64, off: i64, imm: i64) -> Result<Insn, String> {
	...
	Ok(Insn { opc, dst: dst as u8, src: src as u8, off: off as i16, imm: imm as i32 })
}
